...
const studioUrlSchema = z.object({
  url: z.string().url("Please enter a valid URL").min(1, "URL is required"),
})

type StudioUrlForm = z.infer<typeof studioUrlSchema>
...
export default function NewStudioPage() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<StudioUrlForm>({
    resolver: zodResolver(studioUrlSchema),
  })

  const onSubmit = async (data: StudioUrlForm) => {
    ...
    try {
      ...
      router.push(`/studios/${result.studioId}/processing`)
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
      <div style={{ marginBottom: '32px' }}>
        <h1 style={{ fontSize: '30px', fontWeight: 'bold', marginBottom: '8px', color: '#F3F7FF' }}>Create Brand Studio</h1>
        <p style={{ color: '#A8B5CC' }}>
          Enter your website URL and we'll analyze your brand to create your studio
        </p>
      </div>
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              >
                {isSubmitting ? "Analyzing..." : "Create Studio"}
              </button>
              <button
                type="button"
          ...
          <div style={{ marginTop: '24px', padding: '16px', borderRadius: '12px', background: 'rgba(122, 108, 255, 0.1)', border: '1px solid rgba(122, 108, 255, 0.2)' }}>
            ...
            <p style={{ fontSize: '14px', color: '#A8B5CC' }}>
              ...
              You'll review every detected brand value before generating content.
            </p>
          </div>
        </div>
